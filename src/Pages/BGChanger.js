    ...
    return (
        <div className="mx-auto p-4">
            <div className="flex items-center justify-center">
                <div className="bg-white shadow-lg rounded-lg p-6 sm:p-8 w-full max-w-md" style={{ backgroundColor: selectedBgColor }}>
                    <h4 className={`${selectedTextColor} text-2xl sm:text-3xl font-bold text-center mb-6`}>Select your favourite color & see the magic..</h4>
                    <div className="my-4">
                        <select
                            name="color"
                        ...
                        >
                            {colorsList.map((data, index) => (
                                <option key={index} value={JSON.stringify({ bg: data.value, text: data.textColor })}>
                                    {data.title}
                                </option>
                            ))}
                        </select>
                    ...
                    <div className={`${selectedTextColor}`}>
                        ...
                        <span
                            onClick={() => resetChanger()}
                            className={`${selectedTextColor} text-blue-500 hover:cursor-pointer hover:underline`}
                        >
                            Reset Here
                        </span>
                    </div>
